...
const addTask = (e) => {
  if (newTaskName.value.length && selectedGroup.id !== "1") {
    e.preventDefault();
    const newTaskData = {
  ...
  if (selectedGroup.id === "1") {
    e.preventDefault();
    alert("You have to be in a group to add tasks");
  }
};
...
const renderTasks = () => {
  ...
  tasks.forEach((task) => {
    ...
    if (task.group.id === selectedGroup.id || selectedGroup.id === "all") {
      tasksRenderBox.innerHTML += `
      <div class="task">
      <div class="flex items-center gap-3 w-full">
      <div class="done-button ${task.id}">
